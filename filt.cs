...
public class filt : MonoBehaviour
{
    ...
    private float[] acnt=new float[30];
    ...
    void Start()
    {
        ...
        GameObject obj2 = GameObject.Find("GameMaster");
        gamemaster = obj2.GetComponent<GameMaster>();
    ...
    void Update(){
        ...
        if (!flag && updownbar.barY < 0) {
            flag = true;
            if (example.extendedCount == 0) Hz = 130.8F;
            if (example.extendedCount == 1) Hz = 146.8F;
            if (example.extendedCount == 2) Hz = 164.8F;
            if (example.extendedCount == 3) Hz = 174.6F;
            if (example.extendedCount == 4) Hz = 196F;
            if (example.extendedCount == 5) Hz = 220F;
            ampa=1.0F;
        }
        else ampa*=0.9F;
        timeread=Time.time;
    ...
    {   
        float fps=30.0F/(float)(timeread-acnt[fpscnt]);
        acnt[fpscnt]=timeread;
        fpscnt+=1;
        ...
        for (int i = 0; i < sampleCount; i++) // 必要なサンプル数だけ
        {
            ...
            float f=Hz/(float)(43*sampleCount);
